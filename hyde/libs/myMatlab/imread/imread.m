...
if strcmp(classType,'BSONfile')
    import phytoCompute.*;    
    filename = char(filename.getFile());
    fprintf(['BSONfile@phytoGraph \n']);
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% added for routing - IRODS string
fprintf(['imread@phyto\n']);
if isIRODS(filename)
    filename = xfer_get({filename},tempname,0,0);
    filename = filename{1};
    isIR =true;
    fprintf(['fuse@iPlant \n']);
else
...
case 1
    ...
    % Filename only.
    filename = varargin{1};
    if ~ischar(filename) & ~strcmp(class(filename),'phytoCompute.BSONfile')
